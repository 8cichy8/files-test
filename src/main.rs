...
pub fn index(_req: HttpRequest) -> Result<fs::NamedFile> {
    Ok(fs::NamedFile::open("static/index.html")?)
}

pub fn static_files_manual_async(req: HttpRequest) -> impl Future<Item = HttpResponse, Error = ActixError> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(web::resource("/static2/{tail:.*}")
                .route(web::get().to_async(static_files_manual_async))
            )
    }).bind("0.0.0.0:8081")
